function [BFRACM,CSw,GSw,CSo,GSo,i,sQ1,sQ2,sQm,sQd,QWOG]=fun3(RC,Pi,Phi,SW,SO,Cp,PR,TRM,M2FR,WELL,Pw,dt,CR_rc,Qz,Qf,ndt,Pi0,GEOM,Lc,P0,SGM,ft,QWOG)
...
[bAl,bAw,bAo,ql,qw,qo]=Potok_GY0(T_gy,Pgy,Pa,rc_gy,KfwM,KfoM,KfgM,v1,mu,Na,SGM);
[bDl,bDw,bDo,qld,qwd,qod]=Potok_GY0(T_gy_d,Pgy2,Pd,rc_gy_d,KfwD,KfoD,KfgD,v2,mu,Nd,SGM);
...
while fl2<2% & i<3000
    ...
    while flag_gim==1 && kj<2
        kj=kj+1;
        [SGMFR]=SGim([dVa;dVc;dVg;dVd],Sw,So,SGMFR.Mp(:,2),zc,SGMFR.Rs(:,2),rs,SGMFR.Bwog,dPt,PjOld,1,va,vc,vg,vd,vb,dt/ndt);
        PjOld = Pj;
        ...
        [W1,W6,Wo,Wg,W7]=Well_MKT_2(wom(:,2),wom(:,1),Uf(wom(:,3)),Cp(v_a(v1==1)),mu,CpW(wom(:,3)),kfw(va),kfo(va),kfg(va),SGMFR);
        [W1C,W6C,WoC,WgC,W7C]=Well_MKT_2(WELL.WonC(:,2),WELL.WonC(:,1),Uf(WELL.WonC(:,3)),Cp(v_c),mu,CpW(WELL.WonC(:,3)),kfw(vc),kfo(vc),kfg(vc),SGMFR);
        [W1G,W6G,WoG,WgG,W7G]=Well_MKT_2(WELL.WonG(:,2),WELL.WonG(:,1),Uf(WELL.WonG(:,3)),Cp(v_g),mu,CpW(WELL.WonG(:,3)),kfw(vg),kfo(vg),kfg(vg),SGMFR);
        [W1D,W6D,WoD,WgD,W7D]=Well_MKT_2(wod(:,2),wod(:,1),Uf(wod(:,3)),Cp(v_d(v2==1)),mu,CpW(wod(:,3)),kfw(vd),kfo(vd),kfg(vd),SGMFR);
        ...
        [dItime,dItimeW,dItimeO] = NR_Time([dVa;dVc;dVg;dVd],Sw,So,Sw0,So0,SGMFR,va,vc,vg,vd,vb,dt/ndt);
        ...
        [bAl,bAw,bAo,ql,qw,qo]=Potok_GY(T_gy,Pgy,Pa,dPA(v_a),rc_gy,KfwM,KfoM,KfgM,v1,mu,Na,SGMFR,ql,qw,qo);
        [bDl,bDw,bDo,qld,qwd,qod]=Potok_GY(T_gy_d,Pgy2,Pd,dPD(v_d),rc_gy_d,KfwD,KfoD,KfgD,v2,mu,Nd,SGMFR,qld,qwd,qod);
        
        [QWOG.QQm,QQmwo]=QIter(QWOG.QQm,W6,Wo,Wg,dPj(va),wom(:,1),dPw(wom(:,3)),SGMFR);
        [QWOG.QQc,QQcwo]=QIter(QWOG.QQc,W6C,WoC,WgC,dPj(vc),WELL.WonC(:,1),dPw(WELL.WonC(:,3)),SGMFR);
        [QWOG.QQg,QQgwo]=QIter(QWOG.QQg,W6G,WoG,WgG,dPj(vg),WELL.WonG(:,1),dPw(WELL.WonG(:,3)),SGMFR);
        [QWOG.QQd,QQdwo]=QIter(QWOG.QQd,W6D,WoD,WgD,dPj(vd),wod(:,1),dPw(wod(:,3)),SGMFR);
