function [Pj,Swj,Tj,MCpj,p,Q,Pw,PpW,Cw,NDT,dQ,dSS,dt1,V0]=SimT_MKT(PR,C,A2C,G,A2G,dVC,dVG,DATA,WData,GYData,fll,CR_GRUP)
tic
...
[TM,TC,TG,TA2C,TA2G,RC,Txy_GY,Tz_GY]=Pre_fast(A,C,A2C,A2G,C2G,G,Ke,L,B,S,H1,K(:,1),Ke_gy_XY,Ke_gy_Z,BZ,rz,cz,BndXY(p),BndZ(p));
Cw(:,1)=Sw(RC.ACr,1);
Gw(:,1)=Sw(RC.AGr,1);
Pi(1:na,1)=P;
...
%     [G,TG]=GeoMex(G2,TG1,Pi(na+nc+1:na+nc+ng,1),1);
    ...
    [b1gm,b1gc,b1gg,b2gm]=GY_bild(GYData,Pi(1:na,1),Sw(:,1),BXY,BZ,na,nc,ng,Txy_GY,Tz_GY,as,aw,mu);
...
%         find(ata==0)
        ...
        A1=TL-sparse(Won,Won,W1,na,na)-sparse(1:na,1:na,sum(A2CL,2)+sum(A2GL,2)+Clp(va)+sum(b1gm)',na,na);
        ...
        BM=[b1wm;b1wc;b1wg]'+[-b1gm(2,:).*GY_Pz'-b1gm(1,:).*GY_Pxy',b1gc,b1gg]-(Clp.*Pi)';
        BLGY_GIM=-b1gm(2,:).*GY_Pz'-b1gm(1,:).*GY_Pxy'-(Clp(va).*Pi(va))';
    ...
    qgy=b1gm(1,:)'.*(GY_Pxy-Pi(na));
    cwp_gy=mean(dV).*(zc(3)./Bwo(:,1)+zc(1)./Bwo(:,2))*10;
    GY_Pxy=GY_Pxy+qgy./cwp_gy*dt;

    Pw(Qf~=0,ft+1)=Pt(na+nc+ng+1:end);
    Pwt(Qf~=0,t+1)=Pw(Qf~=0,ft+1);
    %% Водонасыщенность
    SCw=[Sw(:,1);Cw(:,t);Gw(:,t)];
    Sw0=SCw;
 ...
 [SCw,SCp,NDT(t),Q1,Q2,Qm1,dSS(t)]=Sat_fast_2(SCw,SCp,RC,TC,TG,TA2C,TA2G,Pi(:,1),PR,ndt,Won,...
     ...
     W7,L,DATA.Lc,DATA.Lg,Ke,Cws,Cwp,BLGY_GIM,Qz(:,ft+1),WonM,nw,b1gm,b2gm,GYData,Clp,ka1);
%  
...
%      W7,L,DATA.Lc,DATA.Lg,Ke,Cws,Cwp,BLGY_GIM,Qz(:,ft+1),WonM,nw,b1gm,Clp);
    ...
    Qm(WonM,:,t+1)=QBild(W1,W6.*A(Won),W7,Pi(1:na,1),Uf(WonM,ft+1),Won,dt,Pw(WonM,ft+1));
