function VZL(DATA,WXY,P,Pb,Sw,So,T,Cp,Nl,pp,Q,SwC,CR_GRUP,pc,NT,XYgy,a0,pb,GYData,XYgy2,dtz,Won3,A2C)
...
Pb=zeros(size(ka,1),size(Pb,2));
T=zeros(size(ka,1),size(P,2));
Sw=zeros(size(ka,1),size(Sw,2));
So=zeros(size(ka,1),size(So,2));
...
pb=reshape(Pb(:,end),size(WZ,1)/Nl,Nl);
sw=reshape(Sw(:,end),size(WZ,1)/Nl,Nl);
so=reshape(So(:,end),size(WZ,1)/Nl,Nl);
sg = 1 - sw - so;
%sg(sg<=0) = 0;
...
plot_fild(x,y,z,pb,Nl,X,Y,WXY,'Давление насыщения',XYgy,a0,'nearest') % 
figure(98),subplot(2,4,3);
%plot_fild(x,y,z,tt,Nl,X,Y,WXY,'Температура',XYgy,a0,'nearest') % 
plot_fild(x,y,z,so,Nl,X,Y,WXY,'Нефтенасыщенность',XYgy,a0,'nearest') % 
figure(98),subplot(2,4,5);
%plot_fild(x,y,z,log10k,Nl,X,Y,WXY,'Проницаемость',XYgy,a0,'nearest') % 
plot_fild(x,y,z,sg,Nl,X,Y,WXY,'Газонасыщенность',XYgy,a0,'nearest') %
