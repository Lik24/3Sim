function [SGM,CMP]=SGimSeqBO(dV,Sw,So,zc,rs,P,Pb,dPb,dPt,dt,V,CMP)
  
  zcb = 0.001;
  
  SGM.dBwP = - CMP.Bw(:,2)*zc(1)./(1 + zc(1)*(P - Pb));
  SGM.dBoP = - CMP.Bo(:,2)*zc(2)./(1 + zc(2)*(P - Pb));
  SGM.dBoPb = zcb*rs./(1 + zc(2)*(P - Pb)) - SGM.dBoP;
  SGM.dBgP = - CMP.Bg(:,2)*zc(3)./(1 + zc(3)*(P - Pb));

  Bw = CMP.Bw(:,2) + SGM.dBwP.*dPt;
  Bg = CMP.Bg(:,2) + SGM.dBgP.*dPt;
  Bo(V.vg,1) = CMP.Bo(V.vg,2) + SGM.dBoP(V.vg).*dPt(V.vg);
  Bo(V.vp,1) = CMP.Bo(V.vp,2) + SGM.dBoP(V.vp).*dPt(V.vp) + SGM.dBoPb(V.vp).*dPb(V.vp);
...
% Rs(V.vp,1) = CMP.Rs(V.vp,2) + rs*dPb(V.vp);
  ...
  Copb(V.vp,1) = -Mp(V.vp).*So(V.vp).*SGM.dBoPb(V.vp)./Bo(V.vp)./Bo(V.vp);
  Cgpb = Mp.*So.*(rs./Bo - Rs.*SGM.dBoPb./Bo./Bo);
 ...
 % SGM.Coso(V.vg,1) = dV(V.vg)./dt.*Coso(V.vg);
  ...
  SGM.Cosw(V.vp,1) = Cosw(V.vp).*dV(V.vp)./dt;
  SGM.Cgsw(V.vp,1) = Cgsw(V.vp).*dV(V.vp)./dt;
