...
%    end;
   ...
   [~,~,~,~,QQ.QQm,QQ.QQmwo]=Well_MKT20(Pi(VEC.va),Pw(WELL.Won(:,3),t),WELL.Won,WELL.Uf(WELL.Won(:,3)),Cp(VEC.va),PR.aw,PR.as,PR,WELL.CpW(WELL.Won(:,3)),CMP,KWOG,VEC.va,na);% W1 - проводимость по всей жидкости, W6 - только для воды, W7 - полимер 
   [W1C,~,~,~,QQ.QQc,QQ.QQcwo]=Well_MKT20(Pi(VEC.vc),Pw(WELL.WonC(:,3),t),WELL.WonC,WELL.Uf(WELL.WonC(:,3)),Cp(VEC.vc),PR.tw,PR.ts,PR,WELL.CpW(WELL.WonC(:,3)),CMP,KWOG,VEC.vc,nc);
   [W1G,~,~,~,QQ.QQg,QQ.QQgwo]=Well_MKT20(Pi(VEC.vg),Pw(WELL.WonG(:,3),t),WELL.WonG,WELL.Uf(WELL.WonG(:,3)),Cp(VEC.vg),PR.tw,PR.ts,PR,WELL.CpW(WELL.WonG(:,3)),CMP,KWOG,VEC.vg,ng);
   [~,~,~,~,QQ.QQd,QQ.QQdwo]=Well_MKT20(Pi(VEC.vd),Pw(WELL.WonD(:,3),t),WELL.WonD,WELL.Uf(WELL.WonD(:,3)),Cp(VEC.vd),PR.tw,PR.ts,PR,WELL.CpW(WELL.WonD(:,3)),CMP,KWOG,VEC.vd,nd);
   ...
   %[Pi,Sw,Pw(:,t+1),CL,GL,Phi,CMP,sQm2,sQd2,sQc2,sQg2,QQ,QQBND,QQoBND]=PressureCalc2(Pi,Sw,Phi,Sw0,Pw(:,t),Cp,TRM,KWOG,KWOG_GY,CMP,RC,WELL,fp,VEC,GEOM,DATA,M2FR,ft,PR,BXYZ,dt,t,Qf,Qz,GYData,BB,QQ,QQBND,QQoBND);
    ...
    %        CL(RC.Cr2+(RC.Cc2-1)*nc),GL(RC.Gr2+(RC.Gc2-1)*ng),dVCG(VEC.vc),dVCG(VEC.vg),W1C,W1G,WELL.WonC,WELL.WonG,1,0);
       ...
       [BFRACM,Pi,Phi,Sw,Sw0,Pw(:,t),CMP,ndt,QQ]=fun2(RC,Pi,Phi,Sw,Cp,PR,TRM,M2FR,WELL,Pw(:,t),dt,CR_rc,Qz(:,ft+1),Qf,1,GEOM,DATA.Lc,CMP,ft,QQ,KWOG);
    ...
    %       nw,Qz(:,ft+1),dt,Qf,GEOM,CMP,BB,TRM,M2FR,PR,fp,BXYZ,QQBND,QQoBND,CR_rc,sQm1,sQd1,QQ,ft,GYData);
       [Pi,Sw,Pw(:,t+1),CL,GL,Phi,CMP,sQm2,sQc2,sQg2,sQd2,QQ,QQBND,QQoBND]=PressureCalcF2(Pi,Sw,Phi,Sw0,Pw(:,t),Cp,TRM,KWOG,KWOG_GY,CMP,RC,WELL,fp,VEC,GEOM,DATA,M2FR,ft,PR,BXYZ,dt,t,Qf,Qz,GYData,BB,QQ,QQBND,QQoBND,1);
    end;
    So = 1 - Sw;
    MSw(:,1)=Sw(VEC.va);
    CSw(:,t+1)=Sw(VEC.vc);
