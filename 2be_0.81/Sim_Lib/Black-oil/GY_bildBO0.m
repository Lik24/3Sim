function [WBND,QQBND,QQwoBND,KWOG_GY]=GY_bildBO0(GY_Data,Pi,Sw,So,Cp,RC,T_GY_A,T_GY_D,PR,CMP,BXYZ,V)
...
function bal=Abra_GY(T_GY,Kfw_xy,Kfo_xy,Kfg_xy,Kfw_z,Kfo_z,Kfg_z,mu,Cp,CMP,v)
...
bal(:,1) = CMP.Cg(v).*Tg_xy + (CMP.Rs(v,2).*CMP.Cor(v) + CMP.Co(v)).*To_xy + CMP.Cw(v).*Tw_xy;
bal(:,2) = CMP.Cg(v).*Tgz + (CMP.Rs(v,2).*CMP.Cor(v) + CMP.Co(v)).*Toz + CMP.Cw(v).*Twz;
bal(:,3)=Tw_xy;
bal(:,4)=Twz;
bal(:,5)=To_xy;
bal(:,6)=Toz;
