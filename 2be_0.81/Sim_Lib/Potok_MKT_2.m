...
TWa = TW.*sparse(r,c,SGM.Cw(c));
TOa = TO.*sparse(r,c,Rs(c)*SGM.Cor(c)+SGM.Co(c));

TW = SGM.Crg(c).*TW + SGM.Crpb(c).*TO - sparse(1:n,1:n,sum(SGM.Crg(c).*TW + SGM.Crpb(c).*TO,2),n,n);

TO = TO - sparse(1:n,1:n,sum(TO,2),n,n);

TWCgs = TWa - sparse(1:n,1:n,sum(TWa,2),n,n);
TOCgs = TOa - sparse(1:n,1:n,sum(TOa,2),n,n);
TG = SGM.Cg(c).*TG - sparse(1:n,1:n,sum(SGM.Cg(c).*TG,2),n,n);
TL = TG + TWCgs + TOCgs;
