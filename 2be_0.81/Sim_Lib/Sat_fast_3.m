function [Sw,So,Cp,sQm2,sQd2,QWOG]=Sat_fast_3(Phi,Pi,Cp,Sw,So,Sw0,So0,RC,KWOG,WELL,BFRACM,...
    nw,Qzm1,dt,Qf,GEOM,SGMMD,BB,TRM,M2FR,PR,fp,BXYZ,QBOUND,CR_cr,Qm,Qd,QWOG,ft,VSAT,GYData)
...
while flag_gim==1 && kj<10
     ...
     [BOUNDFL]=GY_bild(GYData,Pi([va,vd],1),Sw([va,vd],1),Cp([va,vd],1),RC,TRM.Txyz_GY_A,TRM.Txyz_GY_D,PR,SGM);
   ...
   AMOCgs=[AO1Cgs,   A2DOCgs, A2BOCgs;
    ...
    [dItime,dItimeW,dItimeO] = NR_Time(GEOM.dV,Sw,So,Sw0,So0,SGMMD,dt);
   ...
   [QBOUND] = QIterGY(dPt,QBOUND,BOUNDFL,SGMMD,BXYZ,VEC.vb);
   [QWOG.QQm,QQmwo]=QIter(QWOG.QQm,W1,W6,Wo,dPt(va),WELL.Won(:,1),dPw(WELL.Won(:,3)),SGMMD);
   [QWOG.QQd,QQdwo]=QIter(QWOG.QQd,W1D,W6D,WoD,dPt(vd),WELL.WonD(:,1),dPw(WELL.WonD(:,3)),SGMMD); 
  ...
  %Phi(:,3) = Phi(:,3) + dPt(1:na+nc+ng+nd);
   ...
   [SGMMD]=SGim(GEOM.dV,Sw,So,SGMMD.Mp(:,2),PR.zc,SGMMD.Rs(:,2),PR.rs,SGMMD.Bwog,dPt,Pi,1,dt,VSAT);
