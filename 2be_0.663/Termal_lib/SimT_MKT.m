function [XY,KX,Z,Pj,Swj,Tj,MCpj,p,Q,Pw,PpW,Cw,NDT,dQ,dSS,temp_c]=SimT_MKT(PR,C,A2C,G,A2G,dVC,dVG,DATA,WData,GYData,fll)
tic
...
[L,B,S,H1,Con]=Geome3_1(A,XY,Z,H);
...
%     [TC,TG,TA2C,TA2G,WonC(:,2),WonG(:,2)]=temp_CG(fll,TC1,TG1,TA2C1,TA2G1,WonC1,WonG1,t);
    ...
    temp_c(t,1)=full(sum(C(C(:)>0)));
    temp_c(t,2)=full(sum(G(G(:)>0)));
    temp_c(t,3)=full(sum(TC(TC(:)>0)));
    temp_c(t,4)=full(sum(TG(TG(:)>0)));
 ...
 %   Wf(2)=0;
    ...
    [W1,W6,W7]=Well_MKT(Wf,Won,Uf(:,t+1),Sw(:,1),MCp(:,1),aw,as,mu,CpW(:,t));
    [W1C,W6C,W7C]=Well_MKT(WonC(:,2),WonC(:,1),Uf(WonC(:,3),t+1),Cw(:,t),CCp(:,t),tw,ts,mu,CpW(WonC(:,3),t));
    [W1G,W6G,W7G]=Well_MKT(WonG(:,2),WonG(:,1),Uf(WNG,t+1),Gw(:,t),GCp(:,t),tw,ts,mu,CpW(WNG,t));
...
% sum(Pw(:,t+1)<50)
 ...
 qm(WonM,:)=QBild(W1,W6,W7,Pi(1:na,1),Uf(:,t+1),Won,dt,Pw(WonM,t+1));
 qc(WonC(:,3),:)=QBild(W1C,W6C,W7C,Pi(na+1:na+nc,1),Uf(WonC(:,3),t+1),WonC(:,1),dt,Pw(WonC(:,3),t+1));
 qg(WonG(:,3),:)=QBild(W1G,W6G,W7G,Pi(na+nc+1:end,1),Uf(WNG,t+1),WonG(:,1),dt,Pw(WNG,t+1));
 q=qm+qc+qg;
 Qz1=q(:,1)+q(:,2);

 Qf=Qz1;
 [SCw,SCp,NDT(t),Q1,Q2,Qm1,dSS(t)]=Sat_fast_2(SCw,SCp,RC,TC,TG,TA2C,TA2G,TM,Pi(:,1),PR,ndt,Won,Wf,...
     Uf(:,t+1),dt,dVCG,Pw(:,t+1),WonG,CpW(:,t),WonC,Nl,CR_rc,Qz1,Qf,Pi0,TW,W6);
