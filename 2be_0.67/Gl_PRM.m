...
T=10;%100*365;            % Период разработки.
...
aw=[0.07,1,10];                          % Крайняя точка фазовой в порах w-o-g
 
ts=[1,1,1];                               % Фазовые в трещинах  w-o-g
tw=[1,1,10];                            % Крайняя точка фазовой в трещине w-o-g
kms=0*[1,1,1]*1e-3;                       % Коэф. для ур-я форгеймера
...
zc=[1e-6,1e-5,1e-3,1e-6,1e-6,1e-6]*0;     % Сжимаемость вода-нефть-газ-песчаник-глина-трещ            
Bo=[1,1,0.5,1,1];                    % Объёмные коэффициенты

%% Параметры трещины
dh=0.0005;
Kc=1e+4;                                % Проницаемость трещины
