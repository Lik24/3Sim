...
Tiw=Sat_cal(1,1,1,as,aw)*(1-Cpin)./wmu;
Tip=Sat_cal(1,1,1,as,aw)*Cpin./wmu;%Sat_cal(ones(size(Won,1),1).*Cpin,1,1,as,aw)/mu(4);
...
W6=Wf.*(A(Won).*TW.*(Uf==1)+A(Won).*(Tiw+Tip).*(Uf==-1));%
