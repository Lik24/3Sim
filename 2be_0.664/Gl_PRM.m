...
T=30;%365*40;              % Период разработки.
dt=10;                % Шаг по времени % если 0- то динамический шаг
Ta=T;
ndt=1;

%% ГДМ параметры
as=[1,1,1];                             % Фазовые в порах  w-o-g
aw=[1,1,10];                          % Крайняя точка фазовой в порах w-o-g
 
ts=[2,1,1];                               % Фазовые в трещинах  w-o-g
tw=[1,1,10];                            % Крайняя точка фазовой в трещине w-o-g
kms=0*[1,1,1]*1e-3;                       % Коэф. для ур-я форгеймера

mu=[1,100,0.1,1];                       % Вязкости w-o-g-p
   %С      mu
mup=[0,  mu(1);
