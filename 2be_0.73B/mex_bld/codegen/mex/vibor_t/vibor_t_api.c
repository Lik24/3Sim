...
static void m_emlrt_marshallIn(const mxArray *u, const emlrtMsgIdentifier
  *parentId, emxArray_real_T *y);
static void n_emlrt_marshallIn(const mxArray *WoC, const char_T *identifier,
  emxArray_real_T *y);
...
{
  info[0].context =
    "[E]C:/Users/LIK/Documents/MATLAB/Tube_dev/2be_0.660/Sim_Lib/vibor_t.m";
  info[0].name = "min";
  ...
  emxArray_real_T *y)
{
  ...
  thisId.fIdentifier = identifier;
  thisId.fParent = NULL;
  o_emlrt_marshallIn(emlrtAlias(WoC), &thisId, y);
...
void vibor_t_api(const mxArray * const prhs[20], const mxArray *plhs[3])
{
  ...
  emxInit_real_T(&GL, 1, &d_emlrtRTEI, TRUE);
  ...
  c_emlrt_marshallIn(emlrtAlias(prhs[11]), "GL", GL);
  ...
  n_emlrt_marshallIn(emlrtAlias(prhs[16]), "WoC", WoC);
  n_emlrt_marshallIn(emlrtAlias(prhs[17]), "WoG", WoG);
