...
i=i+1;
     ...
     C1=CL1-sparse(1:nc,1:nc,sum(CL1,1)+sum(A2CL,1)+sum(D2CL,1)+Clp(vc)',nc,nc)-sparse(WonC(:,1),WonC(:,1),W1C,nc,nc);
     G1=GL1-sparse(1:ng,1:ng,sum(GL1,1)+sum(A2GL,1)+sum(D2GL,1)+Clp(vg)',ng,ng)-sparse(WonG(:,1),WonG(:,1),W1G,ng,ng);
     
     C2=CW1-sparse(1:nc,1:nc,sum(CW1,1)+sum(A2CW,1)+sum(D2CW,1)+Cwp(vc)',nc,nc)-sparse(WonC(:,1),WonC(:,1),W6C,nc,nc);
     G2=GW1-sparse(1:ng,1:ng,sum(GW1,1)+sum(A2GW,1)+sum(D2GW,1)+Cwp(vg)',ng,ng)-sparse(WonG(:,1),WonG(:,1),W6G,ng,ng);

    [bAl,bAw,bl,bw]=Potok_GY(T_gy,Pgy,Pa,rc_gy,KfwM,KfoM,vc1,mu,Na);
    [bDl,bDw,bld,bwd]=Potok_GY(T_gy_d,Pgy2,Pd,rc_gy_d,KfwD,KfoD,vc2,mu,Nd);
...
Qm(:,:)=Qm+QBild(W1,W6,W7,Pj(va),Uf(wom(:,3)),wom(:,1),dt/ndt,Pw(wom(:,3)),wom(:,3),nw,wna);
Q1(:,:)=Q1+QBild(W1C,W6C,W7C,Pj(vc),Uf(WonC(:,3)),WonC(:,1),dt/ndt,Pw(WonC(:,3)),WonC(:,3),nw,wnc);
Q2(:,:)=Q2+QBild(W1G,W6G,W7G,Pj(vg),Uf(WonG(:,3)),WonG(:,1),dt/ndt,Pw(WonG(:,3)),WonG(:,3),nw,wng);
Qd(:,:)=Qd+QBild(W1D,W6D,W7D,Pj(vd),Uf(wod(:,3)),wod(:,1),dt/ndt,Pw(wod(:,3)),wod(:,3),nw,wnd);
%SCwC(Na+1:Na+nc)-1
ndtI(i)=ndt;
